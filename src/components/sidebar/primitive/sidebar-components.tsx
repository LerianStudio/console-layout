...
const SidebarHeader = React.forwardRef<HTMLDivElement, SidebarHeaderProps>(
  ({ className, collapsed, ...props }: SidebarHeaderProps, ref) => (
    <div
      ...
      className={cn(
        "flex h-[60px] items-center border-b bg-background px-4",
...
const SidebarContent = React.forwardRef<HTMLDivElement, SidebarContentProps>(
  ({ className, ...props }: SidebarContentProps, ref) => (
    <div
      ref={ref}
      className={cn(
        "group flex flex-1 flex-col gap-4 bg-background px-4 pt-4",
...
const SidebarFooter = React.forwardRef<HTMLElement, SidebarFooterProps>(
  ({ className, ...props }: SidebarFooterProps, ref) => (
    <nav
      ref={ref}
      className={cn(
        "flex w-full justify-center border-t bg-background p-4",
        className
      )}
      {...props}
    />
